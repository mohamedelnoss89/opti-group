// ====== OptiSize WhatsApp Bot v4.9 ======
// - Fixed: process.exit(1) instead of (0) so Back4App restarts container
// - Fixed: Clean PID file on reset
// - Fixed: Better MAC error handling with tracking (delete auth after 2 MAC errors)
// - Fixed: Uncaught exception handler prevents crashes
// - Added: /restart endpoint (keeps auth, just reconnects)
...
process.on('uncaughtException', (err) => {
  ...
  try { fs.appendFileSync(path.join(__dirname, 'bot.log'), `[${new Date().toISOString()}] UNCAUGHT: ${err.message}\n`); } catch {}
});

process.on('unhandledRejection', (reason) => {
  console.error('[UNHANDLED REJECTION]', reason);
  try { fs.appendFileSync(path.join(__dirname, 'bot.log'), `[${new Date().toISOString()}] UNHANDLED REJECTION: ${reason}\n`); } catch {}
});
...
function generateActivationCode(phone) {
  ...
  codes[code] = { phone: phone, createdAt: new Date().toISOString(), used: false, type: 'receipt', days: 30 };
...
function verifyCode(code) {
  ...
  if (genCodes[code]) {
    if (genCodes[code].used) return { valid: false, message: 'Code already used' };
    return { valid: true, days: genCodes[code].days || 30, type: genCodes[code].type || 'receipt', phone: genCodes[code].phone };
  }

  const upperCode = code.toUpperCase().trim();
  if (ALL_CODES[upperCode]) {
    const codeInfo = ALL_CODES[upperCode];
    if (codeInfo.usedBy && codeInfo.usedBy.length >= codeInfo.maxUsers) return { valid: false, message: 'Code max uses reached' };
    return { valid: true, days: codeInfo.days, type: codeInfo.type, code: upperCode };
  }
...
const server = http.createServer(async (req, res) => {
  ...
  if (url === '/' || url === '/qr') {
    ...
    if (botConnected) {
      res.end('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"></head><body style="display:flex;justify-content:center;align-items:center;height:100vh;font-family:Arial;background:#1a1a2e;color:#eee"><div style="text-align:center"><h1 style="color:#0f0">&#10004; البوت متصل ويعمل!</h1><p>OptiSize Bot v4.9 - Running</p></div></body></html>');
    } else if (currentQRCode) {
      res.end('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="20"></head><body style="display:flex;justify-content:center;align-items:center;min-height:100vh;font-family:Arial;background:#1a1a2e;color:#eee"><div style="text-align:center;max-width:90%"><h2>&#128241; امسح كود QR بالواتساب</h2><p>واتساب > الإعدادات > الأجهزة المرتبطة > ربط جهاز</p><img src="' + currentQRCode + '" style="border:10px solid white;border-radius:10px;margin:20px;max-width:80%;height:auto"/><p style="color:#aaa">الكود بيتجدد كل 20 ثانية - الصفحة بتتحدث تلقائي</p>' + (currentPairingCode ? '<p style="margin-top:20px">أو استخدم كود الربط: <b style="font-size:24px;color:#0f0">' + currentPairingCode + '</b></p><p style="color:#aaa">واتساب > الإعدادات > الأجهزة المرتبطة > الربط برقم الهاتف</p>' : '') + '</div></body></html>');
    } else {
      res.end('<html><head><meta name="viewport" content="width=device-width,initial-scale=1"><meta http-equiv="refresh" content="5"></head><body style="display:flex;justify-content:center;align-items:center;height:100vh;font-family:Arial;background:#1a1a2e;color:#eee"><div style="text-align:center"><h2>&#9203; جاري انتظار كود QR...</h2><p>البوت بيشتغل، الصفحة هتتحدث تلقائي بعد 5 ثواني</p></div></body></html>');
    }
    return;
  }
  ...
  if (url === '/reset' && method === 'GET') {
    ...
    // exit code 1 so Back4App restarts the container!
    setTimeout(() => process.exit(1), 1000);
    return;
  }

  // ====== Restart WA connection (keeps auth) ======
  if (url === '/restart' && method === 'GET') {
    log('Restart requested - keeping auth');
    stopPresenceKeepAlive();
    ...
    res.writeHead(200, { 'Content-Type': 'application/json' });
    res.end(JSON.stringify({ success: true, message: 'Bot restarting (auth preserved)...' }));
    return;
  }
  ...
  if (url === '/log') {
    try {
      ...
      res.end((fs.existsSync(LOG_FILE) ? fs.readFileSync(LOG_FILE, 'utf-8') : '').slice(-12000));
    ...
    try {
      ...
      req.on('end', () => {
        try {
          ...
          if (!code) { res.writeHead(400, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ valid: false, message: 'Code is required' })); return; }
          const result = verifyCode(code);
          res.writeHead(200, { 'Content-Type': 'application/json' });
          res.end(JSON.stringify(result));
        } catch (e) { res.writeHead(400, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ valid: false, message: 'Invalid request body' })); }
      });
    } catch (e) { res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ valid: false, message: 'Server error' })); }
    return;
  }
    ...
    try {
      ...
      req.on('end', () => {
        try {
          ...
          if (!code) { res.writeHead(400, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, message: 'Code is required' })); return; }
        ...
        } catch (e) { res.writeHead(400, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, message: 'Invalid request body' })); }
      });
    } catch (e) { res.writeHead(500, { 'Content-Type': 'application/json' }); res.end(JSON.stringify({ success: false, message: 'Server error' })); }
    return;
  }
  ...
  if (url === '/api/codes') {
    ...
    res.end(JSON.stringify({
      generated: genCodes,
      builtin: Object.keys(ALL_CODES).map(k => ({ code: k, type: ALL_CODES[k].type, days: ALL_CODES[k].days, maxUsers: ALL_CODES[k].maxUsers, usedCount: (ALL_CODES[k].usedBy || []).length }))
    }, null, 2));
    return;
  }
  ...
  // ====== Default ======
  res.writeHead(200, { 'Content-Type': 'application/json' });
  res.end(JSON.stringify({ status: 'ok', connected: botConnected, version: '4.8.0', geminiReady: !!geminiModel, endpoints: ['/', '/qr', '/status', '/log', '/reset', '/restart', '/api/verify-code', '/api/use-code', '/api/codes'] }));
});
...
async function startWA() {
  if (startWATimer) { clearTimeout(startWATimer); startWATimer = null; }
  ...
  try {
    ...
    sock.ev.on('connection.update', async (update) => {
      ...
      if (qr) {
        ...
        try {
          ...
          try { currentQRCode = 'https://api.qrserver.com/v1/create-qr-code/?size=400x400&data=' + encodeURIComponent(qr); } catch (e2) { log('External QR API also failed: ' + e2.message); }
        }
        ...
        } catch (e) { log('Pairing code error: ' + e.message); }
      }
      ...
      if (connection === 'close') {
        ...
        if (statusCode === 515 || errorMsg.includes('MAC') || errorMsg.includes('mac')) {
          macErrorCount++;
          log('MAC encryption error #' + macErrorCount + ' detected');

          if (macErrorCount >= 2) {
            log('Multiple MAC errors - deleting auth and reconnecting fresh...');
            deleteAuthFolder();
            reconnectAttempts = 0;
            startWATimer = setTimeout(startWA, 5000);
          } else {
            log('First MAC error - trying reconnect with existing auth...');
            startWATimer = setTimeout(startWA, 5000);
          }
          return;
        }
        ...
        if (statusCode !== DisconnectReason.loggedOut && reconnectAttempts < MAX_RECONNECT) {
          ...
          startWATimer = setTimeout(startWA, delay);
        } else if (statusCode === DisconnectReason.loggedOut) {
          log('Logged out (401). Auto-deleting auth and restarting to get fresh QR...');
          deleteAuthFolder();
          reconnectAttempts = 0;
          startWATimer = setTimeout(startWA, 5000);
        }
    ...
    sock.ev.on('messages.upsert', async ({ messages, type }) => {
      try {
        ...
        if (isDuplicate(msgId)) { log('Duplicate: ' + msgId); return; }
        ...
        if (!canRespond(phone)) { log('Cooldown: ' + phone); return; }
    ...
    if (reconnectAttempts < MAX_RECONNECT) {
      reconnectAttempts++;
      startWATimer = setTimeout(startWA, Math.min(5000 * reconnectAttempts, 60000));
    }
  }
}
...
async function analyzeReceiptWithGemini(imageBuffer) {
  if (!geminiModel) throw new Error('Gemini API not initialized. Set GEMINI_API_KEY env variable.');
  const base64Image = imageBuffer.toString('base64');
  const result = await geminiModel.generateContent([{ inlineData: { mimeType: 'image/jpeg', data: base64Image } }, RECEIPT_VERIFY_PROMPT]);
...
function verifyReceiptData(aiResponse) {
  ...
  if (slashMatch) { const d = parseInt(slashMatch[1]); const m = parseInt(slashMatch[2]); let y = parseInt(slashMatch[3]); if (y < 100) y += 2000; dateOk = checkDateMatch(d, m, y); }
  if (!dateOk) { const isoMatch = dateClean.match(/(\d{4})[\/\-](\d{1,2})[\/\-](\d{1,2})/); if (isoMatch) dateOk = checkDateMatch(parseInt(isoMatch[3]), parseInt(isoMatch[2]), parseInt(isoMatch[1])); }
  if (!dateOk) { for (const [monthName, monthNum] of Object.entries(arabicMonths)) { if (aiDate.includes(monthName)) { const dayMatch = aiDate.match(/(\d{1,2})/); const yearMatch = aiDate.match(/(\d{4})/); if (dayMatch) { dateOk = checkDateMatch(parseInt(dayMatch[1]), monthNum, yearMatch ? parseInt(yearMatch[1]) : undefined); } if (dateOk) break; } } }

  const timeClean = aiTime.replace(/\s/g, '');
  const timeOk = timeClean !== '' && aiTime !== 'لا يوجد' && aiTime !== 'لايوجد' && /\d{1,2}[:.]\d{2}/.test(timeClean) && parseInt(timeClean.match(/\d{1,2}/)?.[0] || '99') < 24;
  ...
  return { verified: allOk, reason, details: { type: aiType, keyword: aiKeyword, number: aiNumber, amount: aiAmount, date: aiDate, time: aiTime, method: aiMethod, edited: aiEdited, aiResult } };
}
...
async function handleReceipt(from, phone, msg) {
  ...
  try {
    ...
    if (checkReceiptHash(buf)) {
      userStates[phone] = 'awaiting_receipt';
      await safeSend(from, { text: RECEIPT_REJECTED.replace('{REASON}', 'الإيصال ده تم استخدامه قبل كده! كل إيصال بيتستخدم مرة واحدة بس') });
      return;
    }
    ...
    log('Verification result: ' + (verification.verified ? 'ACCEPTED' : 'REJECTED - ' + verification.reason));

    if (verification.verified) {
      const code = generateActivationCode(phone);
      log('Generated code ' + code + ' for phone ' + phone);
      await saveCodeToApp(code, phone);
      await safeSend(from, { text: RECEIPT_ACCEPTED.replace('{CODE}', code) });
      userStates[phone] = 'active';
    } else {
      userStates[phone] = 'awaiting_receipt';
      await safeSend(from, { text: RECEIPT_REJECTED.replace('{REASON}', verification.reason) });
    }
  } catch (err) {
    log('Receipt handler error: ' + err.message + '\n' + (err.stack || ''));
    userStates[phone] = 'awaiting_receipt';
    await safeSend(from, { text: '❌ حصل خطأ في مراجعة الإيصال.\nجرب تاني بعد شوية.' });
  }
}

log('OptiSize Bot v4.9 loaded. Waiting for WhatsApp connection...');
// Build: 1778616072
